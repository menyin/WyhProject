...
// +----------------------------------------------------------------------
// | Author: 兜兜虫科技 <82550565@qq.com> <http://www.doudouchongkeji.com>
// +----------------------------------------------------------------------
...
function sendMsg($mobile, $params, $tempId)
{
    // @include_once(ROOT_PATH . '/addons/message/SendMessage.php');
    ...
    // 发送模板短信
    $result = $rest->sendTemplateSMS($mobile,$params,$tempId);
...
{
    $cates = app\common\model\Category::where('status', 1)->select();
 ...
 * 获取省份
 */
function getProvinces($isJson = 1)
{
    if (session('?provinces')) {
        $provinces = session('provinces');
    } else {
        $provinces = \think\Db::name('area')->where(['pid'=>0, 'level'=>1])->column('name', 'id');        
    }
...
{
    $cities = \think\Db::name('area')->where(['pid'=>$provinceId, 'level'=>2])->column('name', 'id');
...
{
    $areas = \think\Db::name('area')->where(['pid'=>$cityId, 'level'=>3])->column('name', 'id');
 ...
 * 获取所有城市
 */
function getAllCities($isJson = 1)
{
    if (session('?cities')) {
        $cities = session('cities');
    } else {
        $cities = \think\Db::name('area')->where(['level'=>2])->column('name', 'id');        
    }
 ...
 * 获取所有地区
 */
function getAllAreas($isJson = 1)
{
    if (session('?areas')) {
        $areas = session('areas');
    } else {
        $areas = \think\Db::name('area')->where(['level'=>3])->column('name', 'id');        
    }
...
function getTreeAreas()
{
    if (session("?areas_tree")) {
        $result = session('areas_tree');
    } else {
        $areas = \think\Db::name('area')->select();
        ...
        foreach ($areas as $row) {
            if ($row['pid'] == 0) {
                ...
                foreach ($areas as $row_2) {
                    if ($row_2['pid'] == $row['id']) {
                        $result[$row['id']][$row_2['id']] = $row_2;

                        foreach ($areas as $row_3) {
                            if ($row_3['pid'] == $row_2['id']) {
                                $result[$row['id']][$row_2['id']][$row_3['id']] = $row_3;
                            }
                        }
                    }
                }
